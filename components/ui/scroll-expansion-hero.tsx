...
const ScrollExpandMedia = ({
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      if (mediaFullyExpanded && e.deltaY < 0 && window.scrollY <= 5) {
        ...
        e.preventDefault()
        const scrollDelta = (e as unknown as any).deltaY * 0.0009
    ...
    const handleTouchStart = (e: TouchEvent) => setTouchStartY((e as unknown as any).touches[0].clientY)
    const handleTouchMove = (e: TouchEvent) => {
      if (!touchStartY) return
      const touchY = (e as unknown as any).touches[0].clientY
  ...
  const rawTextTranslateX = scrollProgress * (isMobileState ? 180 : 150)
  // Clamp translation to avoid moving content outside the viewport which can create horizontal scroll
  const textTranslateX = Math.min(rawTextTranslateX, 40)
        ...
        <div className="relative w-full flex flex-col items-center min-h-[100dvh]">
          ...
          >
            <Image
              ...
              width={1920}
              height={1080}
              className="w-full h-[100dvh] max-w-full"
